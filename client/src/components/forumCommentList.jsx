// client/src/components/forumCommentList.jsx
...
const CommentList = ({ comments, forumPostId, updateComments }) => {
  ...
  const handleEdit = async (commentId, content) => {
    try {
      await axios.put(`${apiBaseUrl}/api/forumComments/${forumPostId}/comments/${commentId}`, { content }, {
        headers: { Authorization: `Bearer ${auth.user.token}` },
      });
      const { data: updatedComments } = await axios.get(`${apiBaseUrl}/api/forumComments/${forumPostId}/comments`);
      updateComments(forumPostId, updatedComments);
  ...
  const handleLike = async (commentId) => {
    try {
      await axios.put(`${apiBaseUrl}/api/forumComments/${forumPostId}/comments/${commentId}/like`, {}, {
        headers: { Authorization: `Bearer ${auth.user.token}` },
      });
      const { data: updatedComments } = await axios.get(`${apiBaseUrl}/api/forumComments/${forumPostId}/comments`);
      updateComments(forumPostId, updatedComments);
